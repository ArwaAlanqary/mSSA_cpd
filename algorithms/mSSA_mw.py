...
class MSSA_mw: 
    ...
    def detect(self, ts):
        ...
        self.window_overlap = int(self.cols_ts*self.overlap_ratio)*self.rows
        ...
        singular_cusum_score = np.zeros_like(current_ts[:,0])
        ...
        while t <= len(current_ts) - self.rows:
            ...
            if t <= len(current_ts) - (self.window_size - self.window_overlap):
                test_matrix = current_ts[t-self.window_overlap: t + (self.window_size - self.window_overlap),:].reshape([self.rows, self.cols], order = 'F')
