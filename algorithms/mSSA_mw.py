...
class mssa_mw: 
    ...
    def detect(self, ts):
        ...
        while t <= len(self.ts) - self.rows:
            if rebase: 
                if t > len(self.ts) - self.window_size - self.rows: 
                    break
            ...
            base_matrix = np.reshape(self.ts[t-self.window_size:t], (self.rows, self.cols), order="F")
            base_matrix = self.ts[t-self.window_size:t,:].reshape([self.rows, self.cols], order = 'F')
            base_matrix = base_matrix[:,np.arange(self.cols).reshape([self.no_ts,self.cols_ts]).flatten('F')]
            ...
            if self.rank < 1: 
                ...
                r = self.rank
            perp_basis = U[:, r:]
            distance_shift_c, eps = self._estimate_c(base_matrix, r)
            distance_h = self.distance_threshold * eps

            test_vector = self.ts[t:t+self.rows,:]
            #distance detection 
